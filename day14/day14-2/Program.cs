...
static (char[][], int start) BuildMap(List<(int x, int y)>[] paths)
{
    ...
    var minX = Math.Min(500 - maxY,
        paths.SelectMany(path => path.Select(coords => coords.x)).Aggregate(int.MaxValue, Math.Min) - 1);

    var maxX = Math.Max(500 + maxY,
        paths.SelectMany(path => path.Select(coords => coords.x)).Aggregate(int.MinValue, Math.Max)) + 1;

    paths = paths.Append(Enumerable.Range(minX, maxX - minX + 1).Select(x => (x, maxY)).ToList()).ToArray();
...
static void PrintMap(IReadOnlyList<char[]> map)
{
    ...
    var maxDigits = NumberOfDigits(map.Count);
    for (var y = 0; y < map.Count; ++y)
    {
        var spaces = maxDigits + 1 - NumberOfDigits(y);
        Console.Write($"{y}{new string(Enumerable.Range(0, spaces).Select(_ => ' ').ToArray())}");
