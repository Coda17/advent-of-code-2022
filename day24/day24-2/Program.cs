...
var (there, iT) = Bfs(states, 0, root, goal);
var (back, iB) = Bfs(states, iT, goal, root);
var (thereAgain, _) = Bfs(states, iB, root, goal);

Console.WriteLine($"{there} + {back} + {thereAgain} = {there + back + thereAgain}");

// Item2 is state index.
static (int, int) Bfs(IReadOnlyList<Map> states, int state, (int x, int y) root, (int x, int y) goal)
{
    var q = new Queue<(int x, int y, int s, int d)>();
    q.Enqueue((root.x, root.y, state, 0));
    ...
    IEnumerable<(int x, int y)> Neighbors(Map state, (int x, int y) v)
    {
        ...
        // Don't leave map through root.
        if (v.y != 0 && state[v.y - 1][v.x] == Space.None)
        {
            yield return (v.x, v.y - 1);
        }

        // Don't leave map through goal.
        if (v.y != state.Height - 1 && state[v.y + 1][v.x] == Space.None)
        {
            yield return (v.x, v.y + 1);
        }
